...
const Card = ({ tree }) => {
  const handleCardClick = () => {
    const treePageUrl = `/pohon-deploy/tree/${encodeURIComponent(tree.id)}`;
    window.open(treePageUrl, '_blank');
  };

  return (
    <div className="bg-white shadow-md rounded-md p-4 cursor-pointer" onClick={handleCardClick}>
      <img src={"/pohon-deploy/" + tree.image} alt={tree.name} className="w-full h-40 object-cover rounded-md mb-4" />
